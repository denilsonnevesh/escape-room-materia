...
const BOSS = [
  { q:"Você tem água + areia. Qual sequência é mais adequada para separar a mistura?", options:["Filtração","Imantação","Catação","Sublimação"], answer:0 },
...
function publicPlayers(room){
  return [...room.players.values()].map(p=>({
    id:p.id,name:p.name,score:p.score,xp:p.xp,lives:p.lives,answered:p.answered,progress:p.progress,finished:p.finished
  })).sort((a,b)=>b.score-a.score || b.xp-a.xp);
}
function currentPayload(room){
  const q = room.bossStarted ? BOSS[room.bossIndex] : QUESTIONS[room.index];
  return {
    type:"question",
    index: room.bossStarted ? room.bossIndex : room.index,
    total: room.bossStarted ? BOSS.length : QUESTIONS.length,
    boss: room.bossStarted,
    phase: room.bossStarted ? 6 : q.phase,
    title: room.bossStarted ? "CHEFÃO FINAL" : q.title,
    q:q.q,
    options:q.options,
...
function startQuestion(room){
  ...
  room.timer = setTimeout(()=>finishQuestion(room),90000);
}
function finishQuestion(room){
  if(!rooms.has(room.code)) return;
  clearTimeout(room.timer);
  broadcast(room,{type:"question_end"});
  setTimeout(()=>{
    if(!rooms.has(room.code)) return;
    if(room.bossStarted){
      room.bossIndex++;
      if(room.bossIndex >= BOSS.length){ room.finished=true; broadcast(room,{type:"game_finished",players:publicPlayers(room)}); return; }
    } else {
      room.index++;
...
wss.on("connection",(ws)=>{
  ...
  ws.on("message",(raw)=>{
    ...
    if(msg.type==="create_room"){
      ...
      const room={code:c,teacherToken:t,players:new Map(),index:0,bossStarted:false,bossIndex:0,deadline:0,timer:null,finished:false};
      rooms.set(c,room);
      session.room=c; session.teacher=true;
      send(ws,{type:"room_created",code:c,teacherToken:t});
      send(ws,{type:"teacher_state",players:publicPlayers(room),status:"lobby"});
      return;
    }
    ...
    if(msg.type==="join_room"){
      ...
      if([...room.players.values()].some(p=>p.name.toLowerCase()===name.toLowerCase()))
        return send(ws,{type:"error",message:"Esse nome já está na sala."});
    ...
    if(msg.type==="teacher_reconnect"){
      ...
      session.room=room.code; session.teacher=true;
      send(ws,{type:"teacher_state",players:publicPlayers(room),status:room.finished?"finished":(room.deadline?"playing":"lobby"),deadline:room.deadline});
      if(room.deadline && !room.finished) send(ws,currentPayload(room));
      return;
    }
    if(!room) return;
    if(msg.type==="start_game" && session.teacher){
      ...
      room.index=0; room.bossStarted=false; room.finished=false;
      startQuestion(room); return;
    }
    if(msg.type==="teacher_next" && session.teacher){
      finishQuestion(room); return;
    }
    if(msg.type==="answer" && session.player){
      const p=room.players.get(session.player);
      if(!p || p.answered || p.lives<=0 || room.finished || !room.deadline) return;
      ...
      p.answered=true; p.progress=room.bossStarted?QUESTIONS.length+room.bossIndex+1:room.index+1;
      if(idx===q.answer){
        const base=100, speed=Math.round((remaining/90000)*50);
        const bossBonus=room.bossStarted?100:0;
        const points=base+speed+bossBonus;
        p.score+=points; p.xp+=points;
        send(ws,{type:"answer_result",correct:true,points,total:p.score});
      } else {
      ...
      broadcast(room,leaderboard(room));
      if(allAnswered(room)) finishQuestion(room);
      return;
    }
  });
...
setInterval(()=>{
  ...
  for(const room of rooms.values()){
    if(room.deadline && !room.finished && now>=room.deadline) finishQuestion(room);
  }
},1000);

server.listen(PORT,()=>console.log(`Escape Room online V3 rodando na porta ${PORT}`));
